...
export class DesktopWorkbenchEnvironmentService implements INativeWorkbenchEnvironmentService {
	...
	constructor(
		...
		console.log(
			"[DesktopWorkbenchEnvironmentService] Initializing for Tauri environment",
		);
		...
		this.enableSmokeTestDriver =
			configuration.enableSmokeTestDriver || false;
	...
	private initializePaths(): void {
		console.log(
			"[DesktopWorkbenchEnvironmentService] Initializing Tauri paths...",
		);
		...
		// App settings home
		this.appSettingsHome = URI.file(
			this.configuration.appSettingsHome || "/app/settings",
		);

		// User roaming data home
		this.userRoamingDataHome = URI.file(
			this.configuration.userRoamingDataHome || "/app/roaming",
		);

		// Settings resource
		this.settingsResource = URI.file(
			this.configuration.settingsResource ||
				"/app/settings/settings.json",
		);

		// ARGV resource
		this.argvResource = URI.file(
			this.configuration.argvResource || "/app/settings/argv.json",
		);

		// Backup workspace home
		this.backupWorkspaceHome = URI.file(
			this.configuration.backupWorkspaceHome || "/app/backups",
		);

		// Window logs path
		this.windowLogsPath = URI.file(
			this.configuration.windowLogsPath || "/app/logs",
		);

		// Log file
		this.logFile = URI.file(
			this.configuration.logFile || "/app/logs/main.log",
		);

		// Extension host logs path
		this.extHostLogsPath = URI.file(
			this.configuration.extHostLogsPath || "/app/logs/extensions",
		);
	...
	get crashReporterDirectory(): URI | undefined {
		return this.configuration.crashReporterDirectory
			? URI.file(this.configuration.crashReporterDirectory)
			: undefined;
	}
	...
	get extensionTelemetryLogResource(): URI | undefined {
		return this.configuration.extensionTelemetryLogResource
			? URI.file(this.configuration.extensionTelemetryLogResource)
			: undefined;
	}
	...
	get policyFile(): URI | undefined {
		return this.configuration.policyFile
			? URI.file(this.configuration.policyFile)
			: undefined;
	}
	...
	get extensionDevelopmentLocationURI(): URI[] | undefined {
		return this.configuration.extensionDevelopmentLocationURI?.map(
			(uri: string) => URI.parse(uri),
		);
	}

	get extensionTestsLocationURI(): URI | undefined {
		return this.configuration.extensionTestsLocationURI
			? URI.parse(this.configuration.extensionTestsLocationURI)
			: undefined;
	}
	...
	get logFile(): URI | undefined {
		return this.configuration.logFile
			? URI.parse(this.configuration.logFile)
			: undefined;
	}

	get extHostLogsPath(): URI | undefined {
		return this.configuration.extHostLogsPath
			? URI.parse(this.configuration.extHostLogsPath)
			: undefined;
	}
}
