...
import { Event, Emitter } from '../../../../Dependency/Microsoft/Dependency/Editor/src/vs/base/common/event.js';
...
export class TauriNativeHostService extends Disposable implements INativeHostService {
  ...
  private registerEventListeners(): void {
    console.log('[TauriNativeHostService] Registering Tauri event listeners...');
  ...
  async closeWindow(targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Closing window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error closing window:', error);
      throw error;
    }
  }

  async minimizeWindow(targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Minimizing window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error minimizing window:', error);
      throw error;
    }
  }

  async maximizeWindow(targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Maximizing window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error maximizing window:', error);
      throw error;
    }
  }

  async unmaximizeWindow(targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Unmaximizing window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error unmaximizing window:', error);
      throw error;
    }
  }

  async setFullScreen(fullscreen: boolean, targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Setting fullscreen ${fullscreen} for window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error setting fullscreen:', error);
      throw error;
    }
  }
  ...
  async showItemInFolder(path: string): Promise<void> {
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error showing item in folder:', error);
      throw error;
    }
  }
  ...
  async openExternal(url: string): Promise<boolean> {
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error opening external URL:', error);
      return false;
    }
  }

  async showOpenDialog(options: any): Promise<URI[] | undefined> {
    console.log('[TauriNativeHostService] Showing open dialog with options:', options);
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        input.onchange = () => {
          ...
          resolve(files.map(file => URI.file((file as any).path || file.name)));
        };
        
        input.click();
      });
    } catch (error) {
      console.error('[TauriNativeHostService] Error showing open dialog:', error);
      throw error;
    }
  }

  async showSaveDialog(options: any): Promise<URI | undefined> {
    console.log('[TauriNativeHostService] Showing save dialog with options:', options);
    ...
    try {
      ...
      return new Promise((resolve) => {
        ...
        link.download = options.defaultUri?.path.split('/').pop() || 'file';
        link.href = 'data:text/plain;charset=utf-8,';
        link.click();
        resolve(undefined);
      });
    } catch (error) {
      console.error('[TauriNativeHostService] Error showing save dialog:', error);
      throw error;
    }
  }
  ...
  async setDocumentEdited(edited: boolean, targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Setting document edited ${edited} for window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error setting document edited:', error);
      throw error;
    }
  }

  async setRepresentedFilename(path: string, targetWindowId?: number): Promise<void> {
    console.log(`[TauriNativeHostService] Setting represented filename ${path} for window ${targetWindowId || 'current'}`);
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error setting represented filename:', error);
      throw error;
    }
  }
  ...
  async isAdmin(): Promise<boolean> {
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error checking admin privileges:', error);
      return false;
    }
  }
  ...
  async getWindowCount(): Promise<number> {
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error getting window count:', error);
      return 1;
    }
  }
  ...
  async relaunch(): Promise<void> {
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error relaunching application:', error);
      throw error;
    }
  }
  ...
  async exit(): Promise<void> {
    ...
    try {
      ...
      console.error('[TauriNativeHostService] Error exiting application:', error);
      throw error;
    }
  }
