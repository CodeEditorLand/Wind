...
export class TauriMainProcessService extends Disposable implements IMainProcessService {

  readonly _serviceBrand: undefined;

  constructor(private readonly windowId: number) {
    super();
    console.log(`[TauriMainProcessService] Initialized for window ${windowId}`);
  }
  ...
  getChannel(channelName: string): IChannel {
    console.log(`[TauriMainProcessService] Getting channel: ${channelName}`);
  ...
  registerChannel(channelName: string, channel: IServerChannel): void {
    console.log(`[TauriMainProcessService] Registering channel: ${channelName}`);
  ...
  async invoke<T>(command: string, ...args: any[]): Promise<T> {
    console.log(`[TauriMainProcessService] Invoking command: ${command}`, args);
    ...
    try {
      ...
      console.error(`[TauriMainProcessService] Error invoking command ${command}:`, error);
      throw error;
    }
  }
}
...
class TauriChannel implements IChannel {
  ...
  constructor(private readonly channelName: string, private readonly windowId: number) {
    console.log(`[TauriChannel] Created channel: ${channelName} for window ${windowId}`);
  }

  /**
   * Call a method on the channel
   */
  async call<T>(command: string, arg?: any, cancellationToken?: any): Promise<T> {
    console.log(`[TauriChannel] Calling ${command} on channel ${this.channelName}`);
    ...
    try {
      ...
      console.error(`[TauriChannel] Error calling ${command} on channel ${this.channelName}:`, error);
      throw error;
    }
  }
  ...
  listen<T>(event: string, arg?: any): Event<T> {
    console.log(`[TauriChannel] Listening to event ${event} on channel ${this.channelName}`);
