...
export class PreparationStage {
		...
		try {
			...
			await errorHandler.handle(
				...
				"warning", // Preparation failures can often be recovered
				{
					...
					suggestion:
						"Some preparation steps failed, but workbench may still start",
				},
			);
	...
	private static setupGlobalVariables(): void {
		...
		(window as any)._VSCODE_FILE_ROOT =
			(window as any).vscode?.context?._configuration?.appRoot ||
			"file:///app";
		console.log(
			`[Stage 4] ✓ _VSCODE_FILE_ROOT = ${(window as any)._VSCODE_FILE_ROOT}`,
		);

		// Set global configuration
		(window as any)._WORKBENCH_CONFIGURATION =
			(window as any).vscode?.context?._configuration || {};
		console.log("[Stage 4] ✓ _WORKBENCH_CONFIGURATION set");

		// Set global services
		(window as any)._WORKBENCH_SERVICES =
			(window as any).__SERVICE_COLLECTION__ || {};
	...
	private static setupNLS(config: any): void {
		...
		console.log(
			`[Stage 4] ✓ NLS configured for locale: ${nlsConfig.language}`,
		);
	}
