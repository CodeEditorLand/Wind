...
import type { StageResult, ServiceData } from '../Types/index.js';
...
export class ServicesStage {
  ...
  private static async initializeEffectRuntime(): Promise<any> {
    ...
    try {
      ...
      const runtime = Effect.runSync(
        Effect.launch(coreServicesLayer)
      );
  ...
  static async checkServiceHealth(serviceName: string): Promise<boolean> {
    ...
    try {
      ...
      if (!service) {
        ...
        health.services.set(serviceName, { status: 'not-found', lastChecked: Date.now() });
        return false;
      }
      ...
      console.log(`[Stage 3] ${serviceName} health: ${isHealthy ? '✓ healthy' : '✗ unhealthy'}`);
      return isHealthy;

    } catch (error) {
      console.error(`[Stage 3] ✗ Health check failed for ${serviceName}:`, error);
      health.services.set(serviceName, {
        status: 'error',
  ...
  static async checkAllServicesHealth(): Promise<Map<string, boolean>> {
    ...
    console.log(`[Stage 3] Health check complete: ${healthyCount}/${totalCount} services healthy`);
