...
export class ConfigurationStage {
	...
	private static normalizeConfiguration(config: any): ConfigurationData {
		...
		const normalized: ConfigurationData = {
			...
			userDataPath:
				migratedConfig.userDataPath || "file:///app/user-data",
	...
	private static applyConfigurationMigrations(config: any): any {
		...
		if (configVersion >= currentVersion) {
			console.log(
				`[Stage 2] ✓ Configuration is up to date (v${configVersion})`,
			);
			return config;
		}

		console.log(
			`[Stage 2] Migrating configuration from v${configVersion} to v${currentVersion}`,
		);
	...
	private static applyMigration(config: any, targetVersion: number): any {
		...
		switch (targetVersion) {
			...
			default:
				console.warn(
					`[Stage 2] ⚠ Unknown migration version: ${targetVersion}`,
				);
		}
	...
	private static persistConfiguration(config: ConfigurationData): void {
		console.log(
			"[Stage 2] Persisting configuration to window.vscode.context...",
		);
