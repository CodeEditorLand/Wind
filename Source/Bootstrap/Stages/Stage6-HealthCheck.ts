...
export class HealthCheckStage {
		...
		try {
			...
			await errorHandler.handle(
				...
				"warning", // Health check failures are warnings, not critical
				{
					stage: "Health Check",
					suggestion:
						"Some health checks failed, but workbench may still be functional",
				},
			);
	...
	private static async testCoreFunctionality(): Promise<boolean> {
		...
		console.log(
			`[Stage 6] ✓ Core functionality: ${passed}/${total} tests passed`,
		);
		return passed === total;
	}
		...
		try {
			...
			for (const service of services) {
				try {
					...
					console.warn(
						`[Stage 6] ⚠ Service ${service} not accessible:`,
						error,
					);
				}
			}
			...
			console.log(
				`[Stage 6] ✓ Service access: ${accessible}/${services.length} accessible`,
			);
	...
	private static async testConfigurationAccess(): Promise<boolean> {
		try {
			...
			console.log(
				`[Stage 6] ✓ Configuration access: ${valid}/${requiredFields.length} valid`,
			);
			return valid === requiredFields.length;
		} catch (error) {
			console.error(
				"[Stage 6] ✗ Configuration access test failed:",
				error,
			);
			return false;
		}
	...
	private static async testEditorFunctionality(): Promise<boolean> {
		try {
			...
			if (!workbench) {
				console.error(
					"[Stage 6] ✗ Workbench not available for editor test",
				);
				return false;
			}
			...
			const hasEditorAPI =
				!!(window as any).monaco || !!(window as any).vscode?.editor;
			...
			console.error(
				"[Stage 6] ✗ Editor functionality test failed:",
				error,
			);
			return false;
		}
	...
	private static checkForErrors(): boolean {
		...
		if (criticalErrors.length > 0) {
			console.error(
				`[Stage 6] ✗ Found ${criticalErrors.length} critical errors`,
			);
			return false;
		}
	...
	private static generateHealthReport(results: {
		...
		console.log(
			`[Stage 6] Health Report: ${report.overallHealth.toUpperCase()} (${passed}/${total})`,
		);
		return report;
	}
