...
export class InitializationStage {
	...
	static async execute(): Promise<StageResult> {
		...
		try {
			...
			await errorHandler.handle(this.STAGE_NAME, errorObj, "critical", {
				...
				suggestion:
					"Check VSCode workbench scripts and console for errors",
			});
	...
	private static validatePreconditions(): void {
		...
		if (missing.length > 0) {
			throw new Error(
				`Missing required preconditions: ${missing.join(", ")}`,
			);
		}
	...
	private static async createWorkbench(options: any): Promise<any> {
		...
		for (let i = 0; i < creationStrategies.length; i++) {
			...
			console.log(
				`[Stage 5] 🔄 Attempting strategy ${i + 1}/${creationStrategies.length}: ${strategyName}`,
			);
			...
			try {
				...
				if (workbench) {
					console.log(
						`[Stage 5] ✅ Workbench created using ${strategyName} strategy`,
					);

					// Store globally for future reference
					(window as any).__WORKBENCH_INSTANCE__ = workbench;
					(window as any).__WORKBENCH_CREATION_STRATEGY__ =
						strategyName;
				...
				console.warn(
					`[Stage 5] ⚠ ${strategyName} strategy failed:`,
					error,
				);
				lastError =
					error instanceof Error ? error : new Error(String(error));
		...
		throw (
			lastError ||
			new Error("Failed to create workbench using any strategy")
		);
	}
	...
	private static async createTauriWorkbench(options: any): Promise<any> {
		...
		if (typeof (window as any).createTauriWorkbench === "function") {
			const workbench = await (window as any).createTauriWorkbench(
				tauriOptions,
			);

			if (workbench) {
				console.log(
					"[Stage 5] ✅ Tauri workbench created successfully",
				);
				return workbench;
			}
		}
	...
	private static async createBrowserWorkbench(options: any): Promise<any> {
		...
		if (typeof (window as any).createBrowserWorkbench === "function") {
			const workbench = await (window as any).createBrowserWorkbench(
				browserOptions,
			);

			if (workbench) {
				console.log(
					"[Stage 5] ✅ Browser workbench created successfully",
				);
				return workbench;
			}
		}
	...
	private static validateWorkbenchOptions(options: any): any {
		...
		if (!validated.serviceCollection) {
			console.warn(
				"[Stage 5] ⚠ Service collection not provided, creating minimal one",
			);
	...
	private static async createFallbackWorkbench(options: any): Promise<any> {
		...
		const workbench = {
			...
			get initialized(): boolean {
				return workbenchState.initialized;
			},
			get running(): boolean {
				return workbenchState.running;
			},
			get servicesReady(): boolean {
				return workbenchState.servicesReady;
			},
			get startupTime(): number {
				return workbenchState.startupTime;
			},
			...
			startup: async (): Promise<{
				...
				console.log(
					"[Stage 5] 🚀 Starting advanced fallback workbench...",
				);
				...
				try {
					...
					console.log(
						`[Stage 5] ✅ Advanced fallback workbench started in ${workbenchState.startupTime}ms`,
					);
					...
					console.error(
						"[Stage 5] ❌ Fallback workbench startup failed:",
						error,
					);
			...
			shutdown: async (): Promise<{
				...
				console.log(
					"[Stage 5] 🔄 Shutting down advanced fallback workbench...",
				);
				...
				try {
					...
					console.log(
						`[Stage 5] ✅ Advanced fallback workbench shut down in ${shutdownTime}ms`,
					);
					...
					console.error(
						"[Stage 5] ❌ Fallback workbench shutdown failed:",
						error,
					);
	...
	private static async initializeFallbackServices(): Promise<void> {
		...
		console.log(
			`[Stage 5] 🔄 Available services: ${registeredServices.map((s) => s.name).join(", ")}`,
		);
	 ...
	 */
	private static async initializeWorkbenchServices(
		workbench: any,
	): Promise<void> {
		...
		try {
			...
			if (workbench.initServices) {
				console.log(
					"[Stage 5] 🔄 Phase 1: Using workbench built-in service initialization",
				);
				await workbench.initServices();
				console.log(
					"[Stage 5] ✅ Phase 1: Workbench services initialized",
				);
			} else {
				console.warn(
					"[Stage 5] ⚠ Phase 1: Workbench service initialization not available",
				);

				// Phase 2: Advanced manual service initialization
				console.log(
					"[Stage 5] 🔄 Phase 2: Advanced manual service initialization",
				);
				await this.initializeServicesAdvanced();
				console.log(
					"[Stage 5] ✅ Phase 2: Services initialized manually",
				);
			}
			...
			console.log(
				"[Stage 5] 🔄 Phase 4: Error recovery with fallback services",
			);
	...
	private static async initializeServicesAdvanced(): Promise<void> {
		...
		console.log(
			`[Stage 5] 🔄 Service statistics: ${JSON.stringify(stats)}`,
		);
		...
		for (const serviceName of criticalServices) {
			const isRegistered = serviceAdapter.hasService({
				toString: () => serviceName,
			} as any);

			if (!isRegistered) {
				console.warn(
					`[Stage 5] ⚠ Critical service not registered: ${serviceName}`,
				);
				...
				console.log(
					`[Stage 5] ✅ Critical service available: ${serviceName}`,
				);
			}
		}

		console.log(
			"[Stage 5] ✅ Advanced manual service initialization complete",
		);
	}
	...
	private static createMinimalServiceCollection(): any {
		...
		return {
			set: <T>(id: any, instance: T): void => {
				...
				console.log(
					`[Stage 5] ✅ Service registered: ${id.toString()}`,
				);
			},
			get: <T>(id: any): T => {
				...
				if (!service) {
					console.warn(
						`[Stage 5] ⚠ Service not found: ${id.toString()}`,
					);
				}
				return service as T;
			},
	 ...
	 */
	private static async registerCriticalService(
		serviceName: string,
	): Promise<void> {
		console.log(
			`[Stage 5] 🔄 Registering critical service: ${serviceName}`,
		);
		...
		try {
			...
			await serviceAdapter.registerService(
				serviceId,
				minimalService,
				undefined,
				{
			...
			console.log(
				`[Stage 5] ✅ Critical service registered: ${serviceName}`,
			);
		} catch (error) {
			console.error(
				`[Stage 5] ❌ Failed to register critical service: ${serviceName}`,
				error,
			);
		}
	}
			...
			case "IConfigurationService":
				return {
					...
					updateValue: async (
						...
						console.log(
							`[Stage 5] 🔄 Configuration update: ${key} =`,
							value,
						);
					},
				};
	...
	private static createEmergencyService(serviceName: string): any {
		...
		return {
			...
			getValue: () => undefined,
			log: (message: string) =>
				console.log(`[EMERGENCY:${serviceName}] ${message}`),
		};
	}
	...
	private static async validateServiceHealth(): Promise<void> {
		...
		if (serviceStats.unhealthyServices > 0) {
			console.warn(
				`[Stage 5] ⚠ ${serviceStats.unhealthyServices} unhealthy services detected`,
			);
	...
	private static async restartUnhealthyServices(): Promise<void> {
		...
		for (const service of registeredServices) {
			if (!service.healthy) {
				console.log(
					`[Stage 5] 🔄 Restarting unhealthy service: ${service.name}`,
				);
	...
	private static async restartService(serviceName: string): Promise<void> {
		...
		const serviceAdapter = ServiceAdapter.getInstance();
		serviceAdapter.setServiceHealth(
			{ toString: () => serviceName } as any,
			true,
		);
	...
	private static createEmergencyServiceCollection(): any {
		...
		return {
			...
			has: () => true,
			set: () => {
				console.log("[EMERGENCY] Service registration attempt");
			},
		};
	}
	...
	private static async startWorkbench(workbench: any): Promise<void> {
		...
		try {
			if (workbench.startup) {
				...
				console.warn(
					"[Stage 5] ⚠ Workbench startup method not available",
				);
