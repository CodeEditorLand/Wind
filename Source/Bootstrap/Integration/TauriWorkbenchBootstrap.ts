...
export class TauriWorkbenchBootstrap {
	...
	constructor(config: Partial<ITauriWorkbenchConfig> = {}) {
		...
		console.log(
			"[TauriWorkbenchBootstrap] Initializing Tauri workbench bootstrap",
		);
	}
	...
	async bootstrap(): Promise<ITauriWorkbenchResult> {
		...
		try {
			console.log(
				"[TauriWorkbenchBootstrap] Starting Tauri workbench bootstrap sequence",
			);
			...
			console.log(
				`[TauriWorkbenchBootstrap] Bootstrap completed in ${duration.toFixed(0)}ms`,
			);
	...
	private async initializeServiceMappings(): Promise<void> {
		console.log(
			"[TauriWorkbenchBootstrap] Initializing service mappings...",
		);
		...
		console.log(
			"[TauriWorkbenchBootstrap] Service mappings initialized successfully",
		);
	}
	...
	private async bootstrapCoreInfrastructure(): Promise<IBootstrapResult> {
		console.log(
			"[TauriWorkbenchBootstrap] Bootstrapping core infrastructure...",
		);
		...
		console.log(
			`[TauriWorkbenchBootstrap] Core infrastructure bootstrap ${result.success ? "succeeded" : "failed"}`,
		);
		return result;
	}
	...
	private async createWorkbenchEnvironment(): Promise<any> {
		console.log(
			"[TauriWorkbenchBootstrap] Creating workbench environment...",
		);
	...
	private async initializeWorkbenchServices(workbench: any): Promise<void> {
		console.log(
			"[TauriWorkbenchBootstrap] Initializing workbench services...",
		);
		...
		for (const serviceName of services) {
			try {
				...
				if (service) {
					console.log(
						`[TauriWorkbenchBootstrap] Initialized service: ${serviceName}`,
					);
				} else {
					console.warn(
						`[TauriWorkbenchBootstrap] Service not available: ${serviceName}`,
					);
				}
			} catch (error) {
				console.error(
					`[TauriWorkbenchBootstrap] Failed to initialize service ${serviceName}:`,
					error,
				);
			}
		}
		...
		console.log(
			"[TauriWorkbenchBootstrap] Workbench services initialization complete",
		);
	}
	...
	private async finalizeBootstrap(workbench: any): Promise<void> {
		...
		if (
			...
			typeof workbench.initialize === "function"
		) {
		...
		console.log(
			"[TauriWorkbenchBootstrap] Bootstrap finalization complete",
		);
	}
	...
	private async performFinalChecks(): Promise<void> {
		...
		for (const serviceName of criticalServices) {
			...
			if (!service) {
				throw new Error(
					`Critical service ${serviceName} not available`,
				);
			}
		}
	...
	dispose(): void {
		console.log(
			"[TauriWorkbenchBootstrap] Disposing workbench and services...",
		);
