...
export class ServiceMappingRegistry {
	...
	constructor() {
		console.log(
			"[ServiceMappingRegistry] Initializing service mapping registry",
		);
	...
	private registerDefaultMappings(): void {
		...
		this.registerMapping({
			...
			adapter: {
				...
				validate: (service: any) =>
					service instanceof TauriStorageService,
		...
		this.registerMapping({
			...
			adapter: {
				...
				validate: (service: any) =>
					service instanceof TauriConfigurationService,
		...
		this.registerMapping({
			...
			adapter: {
				...
				validate: (service: any) =>
					service instanceof WindInstantiationService,
		...
		this.registerMapping({
			...
			adapter: {
				adapt: (service: any) => {
					...
					console.log(
						"[ServiceMappingRegistry] Using TauriNativeWindow adapter",
					);
					return service;
				},
		...
		console.log(
			`[ServiceMappingRegistry] Registered ${this.mappings.size} service mappings`,
		);
	}
	...
	registerMapping(mapping: IServiceMapping): void {
		...
		console.log(
			`[ServiceMappingRegistry] Registered mapping: ${mapping.vscodeService} -> ${mapping.windService}`,
		);
	}
		...
		for (const dep of mapping.dependencies) {
			...
			if (depMapping) {
				...
				if (instance) {
					...
					console.warn(
						`[ServiceMappingRegistry] Dependency ${dep} not available for ${mapping.vscodeService}`,
					);
				}
			}
		}
	...
	async initializeServices(): Promise<boolean> {
		...
		const sortedMappings = this.getAllMappings().sort(
		...
		for (const mapping of sortedMappings) {
			try {
				...
				console.error(
					`[ServiceMappingRegistry] Failed to initialize ${mapping.vscodeService}:`,
					error,
				);

				if (!mapping.optional) {
					console.error(
						`[ServiceMappingRegistry] Critical service ${mapping.vscodeService} failed, aborting`,
					);
					return false;
				}
			}
		...
		console.log(
			`[ServiceMappingRegistry] Successfully initialized ${successCount}/${sortedMappings.length} services`,
		);
	...
	private async initializeService(mapping: IServiceMapping): Promise<void> {
		console.log(
			`[ServiceMappingRegistry] Initializing service: ${mapping.vscodeService}`,
		);
		...
		try {
			...
			const existingService = this.getServiceInstance(
				mapping.windService,
			);
			...
			console.log(
				`[ServiceMappingRegistry] Successfully initialized ${mapping.vscodeService}`,
			);
		} catch (error) {
			console.error(
				`[ServiceMappingRegistry] Failed to initialize ${mapping.vscodeService}:`,
		...
		this.serviceInstances.forEach((instance, serviceName) => {
			if (instance.dispose && typeof instance.dispose === "function") {
				try {
					instance.dispose();
					console.log(
						`[ServiceMappingRegistry] Disposed service: ${serviceName}`,
					);
				} catch (error) {
					console.error(
						`[ServiceMappingRegistry] Error disposing service ${serviceName}:`,
						error,
					);
				}
			}
