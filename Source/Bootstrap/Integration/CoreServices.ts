	...
	createConfigurationService(config: any): IVSCodeConfigurationService {
		...
		const service: IVSCodeConfigurationService = {
			...
			onDidChangeConfiguration: (listener: (e: any) => any): any => {
				...
				console.log(
					"[CoreServices] onDidChangeConfiguration listener registered",
				);
				return { dispose: () => {} };
			},
			...
			async updateValue(
				...
				console.log(
					`[CoreServices] Configuration update: ${key} = ${value}`,
				);
	...
	createInstantiationService(): any {
		...
		const service = {
			...
			createInstance<T>(ctor: any, ...args: any[]): T {
				console.log(
					`[CoreServices] Creating instance of: ${ctor.name}`,
				);
