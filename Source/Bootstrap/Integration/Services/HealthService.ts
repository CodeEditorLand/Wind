...
export class HealthService {
    ...
    private async initialize(): Promise<void> {
        try {
            ...
            console.log('[HealthService] Health monitoring service initialized');
        } catch (error) {
    ...
    private async setupEventListeners(): Promise<void> {
        try {
            ...
            console.error('[HealthService] Failed to setup event listeners:', error);
            throw error;
        }
    }
    ...
    private async discoverServices(): Promise<void> {
        try {
            ...
            for (const [serviceName, serviceInfo] of Object.entries(mountainServices)) {
                await this.registerService(serviceName, serviceInfo as any);
            }
            
            console.log(`[HealthService] Discovered ${this.discoveredServices.size} Mountain services`);
        } catch (error) {
     ...
     */
    private async registerService(serviceName: string, serviceInfo: any): Promise<void> {
    ...
    private async checkServiceHeartbeats(): Promise<void> {
        ...
        for (const [serviceName, serviceInfo] of this.discoveredServices.entries()) {
            const timeSinceHeartbeat = now - serviceInfo.lastHeartbeat;
            
            if (timeSinceHeartbeat > this.healthConfig.timeoutThreshold) {
                console.warn(`[HealthService] Service ${serviceName} heartbeat timeout`);
    ...
    private async performHealthChecks(): Promise<void> {
        ...
        for (const [serviceName, serviceInfo] of this.discoveredServices.entries()) {
            try {
                ...
                console.error(`[HealthService] Health check failed for ${serviceName}:`, error);
     ...
     */
    private async checkServiceHealth(serviceName: string, serviceInfo: ServiceInfo): Promise<void> {
        try {
            // Get detailed service status from Mountain
            const detailedStatus = await invoke(`mountain_get_service_status`, { service: serviceName });
            ...
            serviceInfo.metrics = {
                ...serviceInfo.metrics,
                responseTime: detailedStatus.responseTime || serviceInfo.metrics.responseTime,
                errorRate: detailedStatus.errorRate || serviceInfo.metrics.errorRate,
                throughput: detailedStatus.throughput || serviceInfo.metrics.throughput,
                memoryUsage: detailedStatus.memoryUsage || serviceInfo.metrics.memoryUsage,
                cpuUsage: detailedStatus.cpuUsage || serviceInfo.metrics.cpuUsage,
                lastUpdated: Date.now()
            };
            ...
            console.log(`[HealthService] Service ${serviceName} health check: ${serviceInfo.status}`);
        } catch (error) {
            console.error(`[HealthService] Failed to check health for ${serviceName}:`, error);
            throw error;
        }
    }
    ...
    private async attemptServiceRecovery(serviceName: string): Promise<void> {
        ...
        if (currentAttempts >= this.healthConfig.maxRetryAttempts) {
            console.warn(`[HealthService] Max recovery attempts reached for ${serviceName}`);
            return;
        }
        
        console.log(`[HealthService] Attempting recovery for ${serviceName} (attempt ${currentAttempts + 1})`);
        ...
        try {
            ...
            console.log(`[HealthService] Recovery successful for ${serviceName}`);
        } catch (error) {
            console.error(`[HealthService] Recovery failed for ${serviceName}:`, error);
    ...
    private async waitForServiceRecovery(serviceName: string): Promise<void> {
        ...
        while (Date.now() - startTime < maxWaitTime) {
            try {
                const status = await invoke(`mountain_get_service_status`, { service: serviceName });
                
                if (status.status === 'running') {
                    console.log(`[HealthService] Service ${serviceName} recovered`);
                    return;
                }
        ...
        throw new Error(`Service ${serviceName} did not recover within ${maxWaitTime}ms`);
    }
    ...
    private handleMountainServiceStatus(statusUpdate: any): void {
        ...
        if (!serviceInfo) {
            console.warn(`[HealthService] Received status update for unknown service: ${service}`);
            return;
        }
        ...
        console.log(`[HealthService] Service ${service} status updated: ${serviceInfo.status}`);
        
        // Emit appropriate event
        const eventType = serviceInfo.status === ServiceHealthStatus.HEALTHY ? 'service_healthy' :
                         serviceInfo.status === ServiceHealthStatus.DEGRADED ? 'service_degraded' :
                         'service_unhealthy';
    ...
    private handleMountainHealthAlert(alert: any): void {
        ...
        console.warn(`[HealthService] Health alert for ${service}: ${issue} (${severity})`);
    ...
    private handleMountainPerformanceMetrics(metrics: any): void {
        ...
        if (serviceInfo) {
            ...
            console.log(`[HealthService] Updated performance metrics for ${service}`);
        }
    }
    ...
    private emitEvent(event: HealthEvent): void {
        this.eventListeners.forEach(listener => {
            try {
                ...
                console.error('[HealthService] Error in event listener:', error);
            }
        });
    }
    ...
    getSystemHealth(): {
        ...
        const healthyServices = services.filter(s => s.status === ServiceHealthStatus.HEALTHY).length;
        const degradedServices = services.filter(s => s.status === ServiceHealthStatus.DEGRADED).length;
        const unhealthyServices = services.filter(s => s.status === ServiceHealthStatus.UNHEALTHY).length;
