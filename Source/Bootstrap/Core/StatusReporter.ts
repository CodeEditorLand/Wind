...
export class StatusReporter {
	...
	update(update: StatusUpdate): void {
		...
		console.log(
			`[StatusReporter] ${statusEmoji} ${update.stage}: ${update.message}`,
		);
	...
	private updateStageList(currentUpdate: StatusUpdate): void {
		...
		stageList.innerHTML = stages
			.map((stage) => {
				const stageUpdates = this.updates.filter(
					(u) => u.stage === stage,
				);
				...
				const duration = latestUpdate?.duration
					? `(${latestUpdate.duration.toFixed(0)}ms)`
					: "";
	...
	private updatePerformanceMetrics(update: StatusUpdate): void {
		...
		const completedStages = this.updates.filter(
			(u) => u.status === "success",
		).length;
		const failedStages = this.updates.filter(
			(u) => u.status === "error",
		).length;
		const totalDuration = this.updates.reduce(
			(sum, u) => sum + (u.duration || 0),
			0,
		);
	...
	finalize(result: BootstrapResult): void {
      ...
      `;
			...
			this.uiContainer
				.querySelector("#stage-list")
				?.parentNode?.appendChild(summaryDiv);
