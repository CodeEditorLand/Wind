/**
 * @module Bootstrap/Core/BootstrapOrchestrator
 * @description
...
import { PreloadStage } from "../Stages/Stage1-Preload.js";
import { ConfigurationStage } from "../Stages/Stage2-Configuration.js";
import { ServicesStage } from "../Stages/Stage3-Services.js";
import { PreparationStage } from "../Stages/Stage4-Preparation.js";
import { InitializationStage } from "../Stages/Stage5-Initialization.js";
import { HealthCheckStage } from "../Stages/Stage6-HealthCheck.js";
import type {
	BootstrapConfig,
	BootstrapResult,
	StageResult,
...
export class BootstrapOrchestrator {
	...
	static getInstance(): BootstrapOrchestrator {
		if (!BootstrapOrchestrator.instance) {
			throw new Error(
				"BootstrapOrchestrator not initialized. Call initialize() first.",
			);
		}
		return BootstrapOrchestrator.instance;
	}
	...
	async execute(): Promise<BootstrapResult> {
		...
		try {
			...
			for (const stage of this.stages) {
				console.log(
					`[BootstrapOrchestrator] Executing stage: ${stage.name}`,
				);
				...
				if (!result.success && result.critical) {
					console.error(
						`[BootstrapOrchestrator] ✗ Critical failure in stage: ${stage.name}`,
					);
			...
			console.error(
				"[BootstrapOrchestrator] ✗ Fatal error during bootstrap:",
				error,
			);
	...
	private logSummary(
		...
		console.log(
			`Stages Completed: ${results.length}/${this.stages.length}`,
		);
		console.log("");
		...
		results.forEach((result, index) => {
			...
			console.log(
				`  ${index + 1}. ${status} ${result.stage} (${duration}ms)`,
			);
