...
export class ErrorHandler {
	...
	async handle(
		...
		// Log error
		console.error(
			`[ErrorHandler] ${severity.toUpperCase()} error in ${stage}:`,
			error,
		);
	...
	private async attemptRecovery(
		...
		switch (stage) {
			...
			default:
				console.warn(
					`[ErrorHandler] No recovery strategy for ${stage}`,
				);
				return false;
		}
	}
	...
	private async recoverConfigurationError(error: Error): Promise<boolean> {
		...
		try {
			...
			console.error(
				"[ErrorHandler] ✗ Configuration recovery failed:",
				recoveryError,
			);
			return false;
		}
	...
	private async recoverServicesError(error: Error): Promise<boolean> {
		...
		try {
			...
			console.error(
				"[ErrorHandler] ✗ Services recovery failed:",
				recoveryError,
			);
			return false;
		}
	...
	private async recoverInitializationError(error: Error): Promise<boolean> {
		...
		try {
			...
			console.error(
				"[ErrorHandler] ✗ Initialization recovery failed:",
				recoveryError,
			);
			return false;
		}
	...
	private applyFallbackConfiguration(config: any): void {
		...
		if (!vscode || !vscode.context) {
			throw new Error(
				"Cannot apply fallback configuration: window.vscode.context not available",
			);
		}
	...
	private async restartInitializationWithFallback(): Promise<void> {
		console.log(
			"[ErrorHandler] Restarting initialization with fallback...",
		);
