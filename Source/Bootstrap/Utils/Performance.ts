...
export class PerformanceTracker {
  ...
  getSummary(): {
    ...
    const totalDuration = completedMeasurements.reduce((sum, m) => sum + m.duration, 0);
    ...
    return {
      totalDuration,
      averageDuration: completedMeasurements.length > 0 ? totalDuration / completedMeasurements.length : 0,
      fastest: completedMeasurements[0]?.name || 'none',
      slowest: completedMeasurements[completedMeasurements.length - 1]?.name || 'none',
  ...
  measure(name: string, startMark: string, endMark: string): PerformanceEntry | undefined {
    try {
      ...
      console.log(`[Performance] Measure: ${name} (${entry.duration.toFixed(2)}ms)`);
  ...
  getNavigationTiming(): PerformanceNavigationTiming | undefined {
    ...
    return entries.length > 0 ? entries[0] as PerformanceNavigationTiming : undefined;
  }
  ...
  printReport(): void {
    ...
    summary.measurements.forEach(m => {
      console.log(`${m.name}: ${m.duration.toFixed(2)}ms (${m.percentage.toFixed(1)}%)`);
    });
