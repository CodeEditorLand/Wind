...
export class Logger {
   ...
   * Internal log method
   */
  private log(level: 'trace' | 'debug' | 'info' | 'warn' | 'error' | 'critical', message: string, data?: any): void {
   ...
   */
  private integrateWithVSCode(level: string, message: string, data?: any): void {
    ...
    if (vscode && vscode.logger) {
      try {
        ...
        console.log(`[Bootstrap] ${level.toUpperCase()}: ${message}`, data);
      }
    }
  }
  ...
  createWithPrefix(prefix: string): {
    ...
    return {
      trace: (message: string, data?: any) => this.trace(`[${prefix}] ${message}`, data),
      ...
      critical: (message: string, data?: any) => this.critical(`[${prefix}] ${message}`, data)
    };
  }
}