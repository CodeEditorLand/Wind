...
export class StatusReporter {
  ...
  CalculateProgress(completedStages: number, totalStages: number): number {
    ...
    return Math.min(100, Math.max(0, (completedStages / totalStages) * 100));
  }
  ...
  DisplayPerformanceMetrics(metrics: Map<string, number>): void {
    ...
    let metricsElement = document.getElementById('bootstrap-metrics-display');
    ...
    if (!metricsElement) {
      ...
      if (this.statusElement.querySelector('#bootstrap-metrics-toggle')) {
        this.statusElement.insertBefore(metricsElement, this.statusElement.lastChild);
      } else {
        this.statusElement.appendChild(metricsElement);
      }
    }

    let metricsHTML = '<div style="font-weight: 600; margin-bottom: 8px;">Performance Metrics</div>';
  ...
  private formatMetricValue(key: string, value: number): string {
    ...
    } else if (key.toLowerCase().includes('percent') || key.toLowerCase().includes('ratio')) {
