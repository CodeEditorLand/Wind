...
export const SandboxLive = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const awaitReady = Effect.async<SandboxGlobals, SandboxNotReadyError>((resume) => {
      const vscode = (window as any).vscode;
      ...
      window.addEventListener('vscode-wind-preload-ready', handler, { once: true });
      ...
      // Cleanup function
      return Effect.sync(() => {
        window.removeEventListener('vscode-wind-preload-ready', handler);
      });
    }).pipe(
...
export const SandboxMockLive = Layer.succeed(
  Sandbox,
  {
