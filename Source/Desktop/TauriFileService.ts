...
export class TauriFileService {
	...
	private eventListeners: Map<
		string,
		Set<(event: IFileSystemEvent) => void>
	> = new Map();
	...
	async exists(path: string): Promise<boolean> {
		try {
			...
			console.error(
				`[TauriFileService] Error checking existence of ${path}:`,
				error,
			);
			return false;
		}
	...
	async readFile(path: string, encoding: string = "utf-8"): Promise<string> {
		try {
			...
			console.error(
				`[TauriFileService] Error reading file ${path}:`,
				error,
			);
	...
	async readFileBinary(path: string): Promise<Uint8Array> {
		try {
			...
			console.error(
				`[TauriFileService] Error reading binary file ${path}:`,
				error,
			);
	...
	async writeFile(
		...
		try {
			...
			console.error(
				`[TauriFileService] Error writing file ${path}:`,
				error,
			);
	...
	async writeFileBinary(path: string, content: Uint8Array): Promise<void> {
		try {
			...
			console.error(
				`[TauriFileService] Error writing binary file ${path}:`,
				error,
			);
	...
	async createDirectory(
		...
		try {
			...
			console.error(
				`[TauriFileService] Error creating directory ${path}:`,
				error,
			);
	...
	async copy(source: string, target: string): Promise<void> {
		try {
			...
			console.error(
				`[TauriFileService] Error copying ${source} to ${target}:`,
				error,
			);
	...
	async move(source: string, target: string): Promise<void> {
		try {
			...
			console.error(
				`[TauriFileService] Error moving ${source} to ${target}:`,
				error,
			);
	...
	async readDirectory(path: string): Promise<IFileSystemEntry[]> {
		try {
			...
			const fileSystemEntries: IFileSystemEntry[] = entries.map(
				(entry) => ({
					path: entry.path,
					name:
			...
			console.error(
				`[TauriFileService] Error reading directory ${path}:`,
				error,
			);
	...
	async stat(path: string): Promise<IFileSystemEntry> {
		try {
			...
			console.error(
				`[TauriFileService] Error getting stats for ${path}:`,
				error,
			);
	...
	async watch(path: string, recursive: boolean = false): Promise<string> {
		...
		try {
			const watcher = await watch(
				...
				{
					recursive:
						recursive || this.options.enableRecursiveWatching,
	...
	async unwatch(watchId: string): Promise<void> {
		...
		if (watcher) {
			try {
				...
				console.error(
					`[TauriFileService] Error unwatching ${watchId}:`,
					error,
				);
			}
		}
		...
		if (listeners) {
			listeners.forEach((listener) => {
				try {
					...
					console.error(
						`[TauriFileService] Error in event listener for ${path}:`,
						error,
					);
				}
			});
	...
	onFileEvent(
		path: string,
		listener: (event: IFileSystemEvent) => void,
	): void {
	...
	async isDirectory(path: string): Promise<boolean> {
		try {
			...
			console.error(
				`[TauriFileService] Error checking if ${path} is directory:`,
				error,
			);
			return false;
		}
	...
	async isFile(path: string): Promise<boolean> {
		try {
			...
			console.error(
				`[TauriFileService] Error checking if ${path} is file:`,
				error,
			);
			return false;
		}
	...
	async getSize(path: string): Promise<number> {
		try {
			...
			console.error(
				`[TauriFileService] Error getting size of ${path}:`,
				error,
			);
			return 0;
		}
	...
	async getModifiedTime(path: string): Promise<number> {
		try {
			...
			console.error(
				`[TauriFileService] Error getting modified time of ${path}:`,
				error,
			);
			return 0;
		}
	...
	dispose(): void {
		...
		this.watchers.forEach((watcher, watchId) => {
			this.unwatch(watchId).catch((error) => {
				console.error(
					`[TauriFileService] Error disposing watcher ${watchId}:`,
					error,
				);
			});
		});
