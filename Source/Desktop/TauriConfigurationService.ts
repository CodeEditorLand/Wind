...
export interface IConfigurationService {
  ...
  inspect<T>(key: string, scope?: ConfigurationScope): { value?: T; defaultValue?: T };
  keys(): string[];
  ...
  emit(value: T): void {
    this.listeners.forEach(listener => {
      try {
        ...
        console.error('[ConfigurationService] Error in event listener:', error);
      }
    });
  }
}
...
export class TauriConfigurationService implements IConfigurationService {
  private configuration: Map<ConfigurationScope, IConfigurationData> = new Map();
  private changeEventEmitter = new EventEmitter<IConfigurationChangeEvent>();
  private isInitialized = false;

  constructor() {
    console.log('[TauriConfigurationService] Initializing configuration service');
    this.initialize();
  }
  ...
  private async initialize(): Promise<void> {
    try {
      ...
      console.log('[TauriConfigurationService] Configuration service initialized');
    } catch (error) {
      console.error('[TauriConfigurationService] Failed to initialize:', error);
  ...
  private async loadConfiguration(): Promise<void> {
    try {
      const configData = await invoke<Record<string, IConfigurationData>>('get_configuration_data');
      
      if (configData.application) {
        this.configuration.set(ConfigurationScope.APPLICATION, configData.application);
      }
      if (configData.workspace) {
        this.configuration.set(ConfigurationScope.WORKSPACE, configData.workspace);
      }
      if (configData.profile) {
        this.configuration.set(ConfigurationScope.PROFILE, configData.profile);
      }
  ...
  private async saveConfiguration(): Promise<void> {
    if (!this.isInitialized) {
      console.warn('[TauriConfigurationService] Configuration service not initialized');
      return;
    }
   ...
   */
  inspect<T>(key: string, scope: ConfigurationScope = ConfigurationScope.APPLICATION): { value?: T; defaultValue?: T } {
    if (!this.isInitialized) {
      console.warn('[TauriConfigurationService] Configuration service not initialized');
      return {};
    }
  ...
  async reloadConfiguration(): Promise<void> {
    ...
    try {
      ...
      console.log('[TauriConfigurationService] Configuration reloaded successfully');
    } catch (error) {
      console.error('[TauriConfigurationService] Failed to reload configuration:', error);
      throw error;
    }
  }
   ...
   */
  async setScopeData(scope: ConfigurationScope, data: IConfigurationData): Promise<void> {
