...
export class TauriStorageService {
	...
	constructor(private storageName: string = "vscode-workbench") {
		...
		console.log(
			`[TauriStorageService] Initializing storage: ${storageName}`,
		);
		this.initialize();
	}
	...
	private async initialize(): Promise<void> {
		try {
			...
			console.log(
				`[TauriStorageService] Storage initialized: ${this.storageName}`,
			);
		} catch (error) {
			console.error(
				`[TauriStorageService] Failed to initialize storage:`,
				error,
			);
	...
	private async loadStorage(): Promise<void> {
		try {
			...
			if (!fileExists) {
				console.log(
					`[TauriStorageService] Storage file does not exist, creating new: ${this.storagePath}`,
				);
				return;
			}

			const data = await readTextFile(this.storagePath, {
				dir: BaseDirectory.AppData,
			});
			...
			const fileMetadata = await metadata(this.storagePath, {
				dir: BaseDirectory.AppData,
			});
			this.currentUsage = fileMetadata.size || data.length;

			console.log(
				`[TauriStorageService] Loaded ${this.storage.size} entries from storage (${this.currentUsage} bytes)`,
			);
		} catch (error) {
			console.error(
				`[TauriStorageService] Failed to load storage:`,
				error,
			);
			throw error;
		}
	...
	private async saveStorage(): Promise<void> {
		if (!this.isInitialized) {
			console.warn(
				"[TauriStorageService] Storage not initialized, cannot save",
			);
			return;
		}
		...
		try {
			...
			console.log(
				`[TauriStorageService] Saved ${this.storage.size} entries to storage (${dataToSave.length} bytes)`,
			);
		} catch (error) {
			console.error(
				`[TauriStorageService] Failed to save storage:`,
				error,
			);
			throw error;
		}
	...
	async set(key: string, value: any): Promise<void> {
		if (!this.isInitialized) {
			console.warn(
				"[TauriStorageService] Storage not initialized, cannot set value",
			);
			return;
		}
	...
	async delete(key: string): Promise<void> {
		if (!this.isInitialized) {
			console.warn(
				"[TauriStorageService] Storage not initialized, cannot delete value",
			);
			return;
		}
	...
	async clear(): Promise<void> {
		if (!this.isInitialized) {
			console.warn(
				"[TauriStorageService] Storage not initialized, cannot clear",
			);
			return;
		}
	...
	private async checkStorageQuota(dataSize: number): Promise<void> {
		...
		if (totalSize > this.storageQuota) {
			throw new Error(
				`Storage quota exceeded: ${totalSize} bytes exceeds ${this.storageQuota} bytes limit`,
			);
		}
	}
	...
	private async encryptData(data: string): Promise<string> {
		try {
			...
			console.warn(
				"[TauriStorageService] Encryption failed, storing plain text:",
				error,
			);
	...
	private async decryptData(encryptedData: string): Promise<string> {
		try {
			...
			console.warn(
				"[TauriStorageService] Decryption failed, reading plain text:",
				error,
			);
	...
	setQuota(quotaInMB: number): void {
		...
		console.log(
			`[TauriStorageService] Storage quota set to ${quotaInMB}MB`,
		);
	}
