...
export class TauriIPCServer {
	...
	private async setupListeners(): Promise<void> {
		...
		try {
			console.log(
				"[TauriIPCServer] Setting up IPC listeners with Mountain integration...",
			);
			...
			await this.setupChannelListener("mountain-ipc-status", (event) => {
				this.handleConnectionStatus(
					event.payload as { connected: boolean },
				);
			});

			// Listen for Mountain synchronization events
			await this.setupChannelListener(
				"mountain-advanced-sync",
				(event) => {
			...
			console.log(
				`[TauriIPCServer] IPC listeners setup complete in ${Date.now() - startTime}ms`,
			);
			console.log(
				`[TauriIPCServer] Mountain connection status: ${mountainStatus.connected ? "CONNECTED" : "DISCONNECTED"}`,
			);
		...
		if (channelListeners) {
			channelListeners.forEach((listener) => {
				try {
					...
					console.error(
						`[TauriIPCServer] Error in listener for channel ${message.channel}:`,
						error,
					);
				}
			});
	...
	async send(channel: string, data: any): Promise<void> {
		...
		try {
			...
			console.error(
				`[TauriIPCServer] Failed to send message on channel ${channel}:`,
				error,
			);
			this.messageQueue.push(message);
		}
	...
	private async processMessageQueue(): Promise<void> {
		while (this.messageQueue.length > 0 && this.isConnected) {
			...
			try {
				...
				console.error(
					"[TauriIPCServer] Failed to send queued message:",
					error,
				);
	 ...
	 */
	private async setupChannelListener(
		channel: string,
		...
		try {
			...
			console.log(
				`[TauriIPCServer] Listener registered for channel: ${channel}`,
			);
		} catch (error) {
			console.error(
				`[TauriIPCServer] Failed to setup listener for channel ${channel}:`,
				error,
			);
	...
	private handleAdvancedSyncEvent(event: any): void {
		try {
			console.log(
				"[TauriIPCServer] Received Mountain advanced sync event:",
				event,
			);
			...
			switch (event.type) {
				...
				default:
					console.warn(
						"[TauriIPCServer] Unknown sync event type:",
						event.type,
					);
			}
		} catch (error) {
			console.error(
				"[TauriIPCServer] Failed to handle advanced sync event:",
				error,
			);
		}
	}
	 ...
	 */
	private async verifyMountainConnection(): Promise<{
		connected: boolean;
		version?: string;
	}> {
		try {
			const status = await invoke<{
				connected: boolean;
				version: string;
			}>("mountain_get_connection_status");
			return status;
		} catch (error) {
			console.error(
				"[TauriIPCServer] Failed to verify Mountain connection:",
				error,
			);
	...
	private handlePerformanceMetrics(payload: any): void {
		console.log(
			"[TauriIPCServer] Processing performance metrics:",
			payload,
		);
		// Emit to Wind services for processing
