...
export class TauriWorkbench extends Layout implements IWorkbenchLayoutService {
  ...
  private createInstantiationService(serviceCollection: any): any {
    ...
    return {
      invokeFunction: (fn: Function, ...args: any[]) => {
        const accessor = {
          get: (id: string) => {
            console.log(`[TauriWorkbench] Accessing service: ${id}`);
   ...
   * Render workbench
   */
  private renderWorkbench(instantiationService: any, storageService: any, configurationService: any): void {
  ...
  private createParts(): void {
    const parts = [
      { id: WorkbenchParts.TITLEBAR_PART, role: 'none', classes: ['titlebar'] },
      { id: WorkbenchParts.BANNER_PART, role: 'banner', classes: ['banner'] },
      { id: WorkbenchParts.ACTIVITYBAR_PART, role: 'none', classes: ['activitybar', 'left'] },
      { id: WorkbenchParts.SIDEBAR_PART, role: 'none', classes: ['sidebar', 'left'] },
      { id: WorkbenchParts.EDITOR_PART, role: 'main', classes: ['editor'] },
      { id: WorkbenchParts.PANEL_PART, role: 'none', classes: ['panel', 'basepanel'] },
      { id: WorkbenchParts.AUXILIARYBAR_PART, role: 'none', classes: ['auxiliarybar', 'basepanel'] },
      { id: WorkbenchParts.STATUSBAR_PART, role: 'status', classes: ['statusbar'] }
    ];
   ...
   */
  private createPart(id: WorkbenchParts, role: string, classes: string[]): void {
  ...
  private setGlobalLeakWarningThreshold(threshold: number): void {
    console.log(`[TauriWorkbench] Set global leak warning threshold: ${threshold}`);
