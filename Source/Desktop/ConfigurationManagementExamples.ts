 ...
 */
export async function example_backupAndRestore(
...
export async function example_mountainSync(
  ...
  console.log('Last synchronization time:', new Date(lastSyncTime).toISOString());
}
 ...
 */
export async function example_automatedBackup(
  desktopMain: DesktopMain
) {
  console.log('\n=== Example 7: Automated Backup Strategy ===\n');
  ...
  try {
    ...
    if (backups.length > 0) {
      ...
      try {
        ...
        console.error('❌ Restoration also failed:', (restoreError as Error).message);
      }
    }
  }
}
