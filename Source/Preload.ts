...
const ipcRenderer = {
  ...
  removeListener: (channel: string, _listener: (event: unknown, ...args: unknown[]) => void) => {
...
const ipcMessagePort = {
  acquire: (responseChannel: string, nonce: string) => {
    ...
    console.log(`[Preload] MessagePort acquire requested: ${responseChannel}, ${nonce}`);
...
const process = {
  platform: (navigator.platform || 'unknown').toLowerCase().includes('win') ? 'win32' :
            (navigator.platform || 'unknown').toLowerCase().includes('mac') ? 'darwin' : 'linux',
