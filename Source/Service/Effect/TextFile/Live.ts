...
function makeLiveTextFileService(): TextFileService {
	...
	const Service: TextFileService = {
		Read: async (uri) => {
			try {
				const Result = await IPCService.invoke(Channel.TextFileRead)([
					UriToPath(uri),
				]);
		...
		Write: async (uri, content) => {
			try {
				await IPCService.invoke(Channel.TextFileWrite)([
					UriToPath(uri),
		...
		SaveAll: async () => {
			try {
				...
				const DirtyUris = Array.isArray(Result)
