...
export interface PanelService {
	...
	readonly createView: (view: CreatePanelView) => PanelView;
	...
	readonly onActiveViewChanges: (
		listener: (id: string | undefined) => void,
	) => () => void;

	/** Show a panel view */
	readonly showView: (id: string) => void;

	/** Hide a panel view */
	readonly hideView: (id: string) => void;

	/** Toggle a panel view's visibility */
	readonly toggleView: (id: string) => void;

	/** Maximize a panel view to take full height */
	readonly maximizeView: (id: string) => void;

	/** Restore a panel view from maximized state */
	readonly restoreView: (id: string) => void;

	/** Get views by type filter */
	readonly getViewsByType: (type: PanelViewType) => ReadonlyArray<PanelView>;
