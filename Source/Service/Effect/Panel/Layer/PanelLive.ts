...
function makePanelService(): PanelService {
	...
	const OnViewsChanges = (
		listener: (views: ReadonlyArray<PanelView>) => void,
	): (() => void) => {
	...
	const SetActiveView = (Id: string): void => {
		if (!_views.find((v) => v.id === Id))
			throw new PanelViewNotFoundError(Id);
	...
	const ShowView = (Id: string): void => UpdateView(Id, { visible: true });

	const HideView = (Id: string): void => UpdateView(Id, { visible: false });

	const ToggleView = (Id: string): void => {
	...
	const MaximizeView = (Id: string): void => {
		if (!_views.find((v) => v.id === Id))
			throw new PanelViewNotFoundError(Id);
	...
	const RestoreView = (Id: string): void =>
		UpdateView(Id, { maximized: false });

	const GetViewsByType = (Type: PanelViewType): ReadonlyArray<PanelView> =>
		_views.filter((v) => v.type === Type);

	const GetVisibleViews: ReadonlyArray<PanelView> = _views.filter(
		(v) => v.visible,
	);

	const GetMaximizedView: PanelView | undefined = _views.find(
		(v) => v.maximized,
	);
