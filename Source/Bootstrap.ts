...
const GetVerboseLogging = (): boolean => {
	try {
		return Boolean(
			(window as any).__VERBOSE_LOGGING__ ||
			(window as any).__BOOTSTRAP_DEBUG__,
		);
	} catch {
		return false;
	}
...
const GetPauseBetweenStages = (): boolean => {
	try {
		return Boolean(
			(window as any).__PAUSE_BETWEEN_STAGES__ ||
			(window as any).__BOOTSTRAP_DEBUG__,
		);
	} catch {
		return false;
...
const bootstrapEffect = (options: BootstrapOptions) =>
	Effect.gen(function* () {
		...
		const startTime = yield* Effect.clockWith((clock) =>
			clock.currentTimeMillis(),
		);

		yield* telemetry.log(
			"info",
			"[Wind Bootstrap] Atomic bootstrap system starting...",
		);
		yield* telemetry.log(
			"info",
			`[Wind Bootstrap] Debug mode: ${options.debugMode}`,
		);
		...
		yield* withSpan(
			...
			Effect.gen(function* () {
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Stage 1: Waiting for preload...",
				);
				...
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Preload ready, globals available",
				);
				yield* telemetry.recordMetric(
					"stage_preload_duration",
					Date.now() - startTime,
				);
			}),
		);
		...
		yield* withSpan(
			...
			Effect.gen(function* () {
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Stage 2: Telemetry initialized via Effect layer",
				);
			}),
		);
		...
		yield* withSpan(
			...
			Effect.gen(function* () {
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Stage 3: Connecting to Mountain backend...",
				);
				...
				yield* telemetry.log(
					"info",
					`[Wind Bootstrap] Connected to Mountain v${version}`,
				);
				yield* telemetry.recordMetric(
					"stage_mountain_connect_duration",
					Date.now() - startTime,
				);
			}),
		);
		...
		yield* withSpan(
			...
			Effect.gen(function* () {
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Stage 4: Fetching configuration...",
				);
				...
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Configuration applied",
				);
				yield* telemetry.recordMetric(
					"stage_configuration_duration",
					Date.now() - startTime,
				);
			}),
		);
		...
		yield* withSpan(
			...
			Effect.gen(function* () {
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Stage 5: Initializing workbench environment...",
				);
				...
				yield* telemetry.log(
					"info",
					"[Wind Bootstrap] Workbench environment ready",
				);
				yield* telemetry.recordMetric(
					"stage_workbench_init_duration",
					Date.now() - startTime,
				);
			}),
		);
		...
		const endTime = yield* Effect.clockWith((clock) =>
			clock.currentTimeMillis(),
		);
		...
		yield* telemetry.log(
			"info",
			"[Wind Bootstrap] Bootstrap process completed",
		);
		yield* telemetry.log(
			"info",
			"[Wind Bootstrap] ✓ All stages completed successfully",
		);
		yield* telemetry.log(
			"info",
			`[Wind Bootstrap] Duration: ${totalDuration}ms`,
		);
		yield* telemetry.log(
			"info",
			"[Wind Bootstrap] ===============================================",
...
const handleBootstrapError = (
	...
	Effect.gen(function* () {
		...
		yield* Effect.sync(() => {
			window.dispatchEvent(
				new CustomEvent("vscode-wind-bootstrap-error", {
...
console.log(
	"[Wind Bootstrap] Bootstrap entry point loaded (Effect-TS version)",
);
...
const runnable = bootstrapProgram.pipe(Effect.provide(TauriLiveLayer));
...
export const bootstrap = async (options: any): Promise<any> => {
	console.warn(
		"[Wind Bootstrap] Legacy bootstrap called - use Effect-TS version instead",
	);
