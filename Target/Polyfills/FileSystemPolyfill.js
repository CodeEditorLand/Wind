async function t(e, i = {}) {
	try {
		if (typeof window.__TAURI__?.invoke < "u")
			return await window.__TAURI__.invoke(e, i);
		if (typeof window.TAURI?.invoke < "u")
			return await window.TAURI.invoke(e, i);
		throw new Error(`Tauri invoke not available for command: ${e}`);
	} catch (r) {
		throw (
			console.error(
				`[FileSystemPolyfill] Tauri invoke failed for ${e}:`,
				r,
			),
			r
		);
...
async function a(e, i) {
	console.log(`[FileSystemPolyfill] readFile: ${e}`);
	const r = typeof i == "string" ? i : (i?.encoding ?? "utf8");
	try {
		const n = await t("file:read", {
			path: e,
			encoding: r === null ? "base64" : r,
		});
		return r === null ? Buffer.from(n, "base64") : n;
	} catch (n) {
		const o = n instanceof Error ? n : new Error(String(n));
		throw (
			console.error(`[FileSystemPolyfill] readFile error: ${e}`, o),
			o
		);
	}
}
async function f(e, i, r) {
	console.log(`[FileSystemPolyfill] writeFile: ${e}`);
	let n = "utf8";
	typeof r == "string" ? (n = r) : r && (n = r.encoding ?? "utf8");
	let o;
	Buffer.isBuffer(i) ? (o = i.toString(n ?? "utf8")) : (o = i);
	try {
		await t("file:write", { path: e, data: o, encoding: n });
	} catch (c) {
		const b = c instanceof Error ? c : new Error(String(c));
		throw (
			console.error(`[FileSystemPolyfill] writeFile error: ${e}`, b),
			b
		);
	}
}
async function u(e) {
	console.log(`[FileSystemPolyfill] unlink: ${e}`);
	try {
		await t("file:delete", { path: e, recursive: !1 });
	} catch (i) {
		const r = i instanceof Error ? i : new Error(String(i));
		throw (console.error(`[FileSystemPolyfill] unlink error: ${e}`, r), r);
	}
}
async function d(e, i) {
	(console.log(`[FileSystemPolyfill] rm: ${e}`),
		(opts = { recursive: !1, force: !1, ...i }));
	try {
		await t("file:delete", {
			path: e,
		...
		if (!opts.force) {
			...
			throw (console.error(`[FileSystemPolyfill] rm error: ${e}`, n), n);
		}
	}
}
async function m(e, i) {
	console.log(`[FileSystemPolyfill] rename: ${e} -> ${i}`);
	try {
		await t("file:move", { from: e, to: i });
	} catch (r) {
		const n = r instanceof Error ? r : new Error(String(r));
		throw (console.error(`[FileSystemPolyfill] rename error: ${e}`, n), n);
	}
}
async function w(e, i, r) {
	console.log(`[FileSystemPolyfill] copyFile: ${e} -> ${i}`);
	try {
		await t("file:copy", { from: e, to: i });
	} catch (n) {
		const o = n instanceof Error ? n : new Error(String(n));
		throw (
			console.error(`[FileSystemPolyfill] copyFile error: ${e}`, o),
			o
		);
	}
}
async function y(e, i) {
	console.log(`[FileSystemPolyfill] mkdir: ${e}`);
	let r = { recursive: !1 };
	typeof i == "boolean"
		? (r.recursive = i)
		: typeof i == "number"
			? ((r.recursive = !1), (r.mode = i))
			: i && ((r.recursive = i.recursive ?? !1), (r.mode = i.mode));
	try {
		await t("file:mkdir", {
			path: e,
		...
		const o = n instanceof Error ? n : new Error(String(n));
		throw (console.error(`[FileSystemPolyfill] mkdir error: ${e}`, o), o);
	}
}
async function g(e) {
	console.log(`[FileSystemPolyfill] rmdir: ${e}`);
	try {
		await t("file:delete", { path: e, recursive: !1, is_rmdir: !0 });
	} catch (i) {
		const r = i instanceof Error ? i : new Error(String(i));
		throw (console.error(`[FileSystemPolyfill] rmdir error: ${e}`, r), r);
	}
}
async function v(e, i) {
	console.log(`[FileSystemPolyfill] readdir: ${e}`);
	try {
		const r = i?.withFileTypes ?? !1,
			n = await t("file:readdir", { path: e });
		return r
			? n.map((o) => p(o.name, `${e}/${o.name}`, !o.is_file))
			: n.map((o) => o.name);
	} catch (r) {
		const n = r instanceof Error ? r : new Error(String(r));
		throw (console.error(`[FileSystemPolyfill] readdir error: ${e}`, n), n);
	}
}
async function s(e) {
	console.log(`[FileSystemPolyfill] stat: ${e}`);
	try {
		const i = await t("file:stat", { path: e });
		return k(i);
	} catch (i) {
		const r = i instanceof Error ? i : new Error(String(i));
		throw (console.error(`[FileSystemPolyfill] stat error: ${e}`, r), r);
	}
}
async function F(e) {
	console.log(`[FileSystemPolyfill] exists: ${e}`);
	try {
		return (await s(e), !0);
	} catch {
		return !1;
...
function S() {
	if (!(typeof window > "u")) {
		...
		((window.__FILE_SYSTEM_POLYFILL_INSTALLED__ = !0),
			...
			(window.fs = l),
			(window.require = B()),
			typeof window.vscode < "u" && (window.vscode.fs = l),
			console.log(
				"[FileSystemPolyfill]\u2001\u2713 Node.js fs module polyfill installed",
			));
	}
}
function B() {
...
typeof window < "u" && S();
export { U as default, S as installFileSystemPolyfill };
